...
BENCHMARK_DIR=/scratch/${USER}/kynema-sgf-benchmark/kynema-sgf-benchmark-gpu
...
cat >build-kynema-sgf-benchmark-gpu.sh <<'EOL'
#!/bin/bash -l
set -ex
module load cuda
...
cat >run-kynema-sgf-benchmark-gpu.sh <<'EOL'
#!/bin/bash -l
...
# Build Kynema-SGF
bash build-kynema-sgf-benchmark-gpu.sh

# Submit run script
sbatch -J kynema-sgf-benchmark-gpu-1 -N 1 run-kynema-sgf-benchmark-gpu.sh; for i in {1..5}; do sbatch -J kynema-sgf-benchmark-gpu-$((2**i)) -N $((2**i)) run-kynema-sgf-benchmark-gpu.sh; done
